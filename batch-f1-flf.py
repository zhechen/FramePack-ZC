"""Batch FramePack sampling with first/last frame anchoring support."""
...
parser.add_argument("--start-frame", dest="start_frame", help="Optional path to anchor the first frames")
parser.add_argument("--end-frame", dest="end_frame", help="Optional path to anchor the last frames")
parser.add_argument("--endpoint-frames", type=int, default=8, help="How many frames should be influenced by the endpoints")
parser.add_argument(
    "--endpoint-strength",
    type=float,
    default=0.65,
    help="Strength of the endpoint blending (0-1)",
)
parser.add_argument(
    "--endpoint-schedule",
    choices=("linear", "cosine"),
    default="cosine",
    help="Scheduling curve for endpoint blending",
)
parser.add_argument(
    "--endpoint-mode",
    choices=("blend", "lock"),
    default="blend",
    help="Blend mixes latents while lock replaces them",
)
...
def worker(
    ...
    end_frame=None,
    endpoint_frames=8,
    endpoint_strength=0.65,
    endpoint_schedule="cosine",
    endpoint_mode="blend",
):
    ...
    target_total_frames = max(int(round(total_second_length * 30)), 1)
    ...
    try:
        ...
        start_frame_path = start_frame or input_path
        if start_frame_path or end_frame:
            endpoint_pipeline = EndpointPipeline(vae)
            endpoint_cfg = {
            ...
            if endpoint_context is not None and torch.cuda.is_available():
                context_device = gpu
                for key in ('z_start', 'z_end', 'schedule'):
                    value = endpoint_context.get(key)
                    if isinstance(value, torch.Tensor):
                        endpoint_context[key] = value.to(device=context_device)
        ...
        for section_index in range(total_latent_sections):
            ...
            if endpoint_context is not None:
                endpoint_context['frame_idx'] = max(total_generated_latent_frames - 1, 0)
                endpoint_context['total_frames'] = target_total_frames
                if torch.cuda.is_available():
                    for key in ('z_start', 'z_end', 'schedule'):
                        value = endpoint_context.get(key)
                        if isinstance(value, torch.Tensor) and value.device != gpu:
                            endpoint_context[key] = value.to(device=gpu)
